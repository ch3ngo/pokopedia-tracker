...
export function HabitatCard({ habitat, progress, allPokemon, onUpdate, lang }: Props) {
  ...
  const isComplete = spawnPokemon.length > 0 && spawnPokemon.every((p) => attracted.includes(p.id));
  ...
  const completionPct =
    spawnPokemon.length > 0 ? Math.round((attracted.length / spawnPokemon.length) * 100) : 0;
      ...
      <div className="p-4">
        ...
        {isBuilt && spawnPokemon.length > 0 && (
          <div className="mb-3">
            <div className="flex justify-between text-xs text-gray-400 mb-1">
              ...
              <span>{attracted.length}/{spawnPokemon.length}</span>
            </div>
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {isBuilt ? t("habitatdex.markBuilt") : t("habitatdex.markBuilt")}
          </button>
          {isBuilt && spawnPokemon.length > 0 && (
            <button
        ...
        {expanded && isBuilt && (
          <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
            ...
            <div className="grid grid-cols-4 gap-2">
              {spawnPokemon.map((p) => {
                const isAttracted = attracted.includes(p.id);
                const pName = lang === "es" ? p.name_es : p.name_en;
                return (
                  <button
                    key={p.id}
                    onClick={() => toggleAttracted(p.id)}
                    title={pName}
                    className={`flex flex-col items-center gap-0.5 p-1 rounded-xl transition-all border
                      ${isAttracted
